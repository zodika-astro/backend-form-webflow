 ...
 * ----------------------------------------------
 * Default: reads from environment variables (and Docker-style *_FILE).
 * Optional adapters (lazy): GCP Secret Manager, AWS Secrets Manager, HashiCorp Vault.
 *
 * Design goals:
 * - Never log secret values.
 * - Work by default with ENV (no extra deps).
 * - Safe in production: cache with TTL, consistent error messages.
 * - Extensible: switch provider via SECRET_PROVIDER=env|file|gcp|aws|vault
 *
 ...
 * - ENV: process.env.NAME or process.env.NAME_FILE / NAME__FILE (Docker secrets)
 * - FILE: SECRETS_DIR=/var/run/secrets -> reads `${SECRETS_DIR}/${name}`
 *
 * WARNING:
 * - GCP/AWS/Vault adapters require their SDKs/creds if you enable them.
 * - If adapter fails or isn’t available, we fall back to ENV to avoid downtime
 *   (and surface a clear warning once).
 */
...
const DEFAULT_TTL_MS = Number(env.SECRET_CACHE_TTL_MS || 5 * 60 * 1000); // 5 min
...
async function get(name, { cacheTtlMs = DEFAULT_TTL_MS, required = isProd } = {}) {
  ...
  if ((value == null || value === '') && required) {
    // Do not leak secret name semantics in production logs beyond key name
    throw new Error(`Missing required secret: ${name}`);
  }

  cache.set(name, { value, expiresAt: t + cacheTtlMs });
  return value;
}
...
// ENV provider (supports Docker secrets via *_FILE / *__FILE)
...
// FILE provider (mount a secrets directory; e.g., Kubernetes, Docker)
function fromFile(name) {
  try {
    const p = path.join(FILE_DIR, name);
...
// Requires: @google-cloud/secret-manager and proper SA credentials on the runtime
...
// Requires: @aws-sdk/client-secrets-manager and IAM permissions
...
// HashiCorp Vault (optional; simple KV v2 read via native fetch)
// Requires: VAULT_ADDR, VAULT_TOKEN, and (optionally) VAULT_KV_MOUNT=secret
async function fromVault(name) {
  try {
    ...
    // Try a sensible default: accept either { value: "..." } or { [name]: "..." }
